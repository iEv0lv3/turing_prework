...
class WerewolfTest < Minitest::Test
  ...
  def test_consumes_a_victim
    werewolf = Werewolf.new("Yoda", "Endor")
    victim = Victim.new

    werewolf.change!
    werewolf.consume!(victim)
    assert werewolf.victims >= 1
  end

  def test_cannot_consume_victim_if_in_human_form
    werewolf = Werewolf.new("Jon Snow", "Winterfell")
    victim = Victim.new
  ...
  def test_a_werewolf_who_has_consumed_a_victim_makes_the_victim_dead
    werewolf = Werewolf.new("Bran", "The Best Story")
    victim = Victim.new
